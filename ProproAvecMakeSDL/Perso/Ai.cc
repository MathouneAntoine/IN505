...
bool Ai::live_Ai(vector < Objects * >&obj, Character & p)
{
    ...
    int i = rand() % 4; // On prend une valeur alatoire pour l'orientation de l'ia
    if (i == 0) {
	this->orientation = 0;
	Translate(0, obj, p);
	return true;
...
void Ai::Translate(int direction, vector < Objects * >&obj, Character & p) //Si il n'y a pas de collision l'ia se déplace sinon il attaque
{
    switch (direction) {
    case 0:
	if (collision (cerc->getCenter().getX() + cerc->getDiametre() / 2,(cerc->getCenter().getY() - cerc->getDiametre() / 2 - 2), obj, 0, p)
	    && collision(cerc->getCenter().getX() - cerc->getDiametre() / 2, (cerc->getCenter().getY() - cerc->getDiametre() / 2 - 2), obj, 0, p)) {
	    cerc->deplacer(0, -2);
	} else
    ...
    case 90:
	if (collision (cerc->getCenter().getX() + cerc->getDiametre() / 2 + 2, (cerc->getCenter().getY() + cerc->getDiametre() / 2), obj, 0, p)
	    && collision(cerc->getCenter().getX() + cerc->getDiametre() / 2 + 2, (cerc->getCenter().getY() - cerc->getDiametre() / 2), obj, 0, p)) {
	    cerc->deplacer(2, 0);
	} else
	    Attack(this->orientation, weaponRange, obj, weaponDamage, p);
